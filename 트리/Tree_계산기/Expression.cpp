...
{
	stack<Term> tempStack;							//연산자만 임시로 저장해두는 stack
	...
	for (int i = 0; i < numOfTerms; ++i)
	{
		...
		else
		{
			...
			{
				while (tempStack.top().token.opr != '(')			//여는괄호를 만날때까지 pop
				{
...
void Expression::setExpressionTree(Tree<Term> &tree)
{
	...
	for (int i = 0; i < numOfTerms; ++i)
	{
		...
		if (exp[i].type == NUMBER)	//숫자
		{
			s.push(node);
		}
		else						//연산자
		{
			if (checkOrderOpr(exp[i].token.opr) == UNARY_ORDER)	//단항연산자
			{
